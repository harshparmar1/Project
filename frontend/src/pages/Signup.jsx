...
import { signup, getDepartments } from '../services/api';
import { Calendar, User, Mail, Lock, Building2, Key, AlertCircle, CheckCircle, ArrowRight, ChevronDown, Clock, BookOpen, GraduationCap, Bell, Pencil, Trophy, Award } from 'lucide-react';
...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!fullName || !email || !password || !department || !departmentCode) {
    ...
    try {
      ...
      console.error(err);
      setError(
        err.response?.data?.detail || 'Signup failed. Please verify the code and try again.'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-200 via-sky-100 to-blue-300 text-slate-800 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center gap-3 mb-8">
          ...
          <p className="text-sm text-slate-500 text-center">Join your department&apos;s scheduling portal</p>
        </div>
        ...
        {success && (
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="mb-6 p-4 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-600 flex items-start gap-2.5 text-sm"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <motion.div variants={itemVariants} className="space-y-1">
            ...
            <div className="relative group">
              <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-primary-500 transition-colors duration-200" />
              <input
                type="text"
                placeholder="Dr. Jane Doe"
                value={fullName}
                onChange={(e) => setFullName(e.target.value)}
                className="w-full bg-slate-50/50 border border-slate-200 hover:border-slate-300 rounded-xl py-2.5 pl-10.5 pr-4 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 placeholder:text-slate-400 focus:bg-white focus:shadow-sm"
              />
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="space-y-1">
            ...
            <div className="relative group">
              ...
              <select
                ...
                className="w-full bg-slate-50/50 border border-slate-200 hover:border-slate-300 rounded-xl py-2.5 pl-10.5 pr-10 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 appearance-none focus:bg-white focus:shadow-sm"
              >
          ...
          <motion.div variants={itemVariants} className="space-y-1">
            ...
            <div className="relative group">
              ...
              <input
                type="text"
                placeholder="Code from admin (e.g. DS2026)"
        ...
        <p className="text-center text-sm text-slate-500 mt-6">
          ...
          <Link to="/login" className="text-primary-600 hover:text-primary-500 font-semibold hover:underline">
            Sign in here
          </Link>
        </p>
      </motion.div>
